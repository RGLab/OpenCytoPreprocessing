...
importGatingSet <- function( labkey.url.base, labkey.url.path, path, analysisName = NULL, analysisDescription = NULL ){
    ...
    tryCatch({
        ...
        if ( length(colNames) > 1 ){
            strngStudyVars <- paste0( 'Sample/', colNames[2:length(colNames)] );
            colnames(meta) <- c( colNames[1], strngStudyVars );
        ...
        rowIds <- labkey.selectRows(
            ...
            schema      = 'flow',
            query       = 'FCSFiles',
            colFilter   = makeFilter( c( 'Name', 'IN', paste( meta$Name, collapse = ';' ) ), c( 'Run/ProtocolStep', 'EQUAL', 'Keywords' ) ),
