 ...
 *   ComboFieldBox
 * 
 *  http://www.sencha.com/forum/showthread.php?195773-Ext.ux.ComboFieldBox-intuitive-multi-select-combobox-for-4.1&p=78026
 *  https://github.com/krikrou/ComboFieldBox
 */
Ext4.define('Ext4.ux.ComboFieldBox', {
    extend : 'Ext.form.field.ComboBox',
	alias : 'widget.combofieldbox', 
	//requires: ['Ext4.ux.ComboView'],
	multiSelect: true,
	/**
 	 * @cfg
	 ...
	 */
	trigger2Cls	: Ext4.baseCSSPrefix + 'form-combo-trigger',
    ...
    initComponent: function () {
        ...
        me.listConfig = Ext4.apply(me.listConfig || {}, selModel);
        if(me.iconClsField || me.descField) {
            Ext4.apply(me.listConfig, {
    ...
    createValueStore: function() {
        return this.valueStore = new Ext4.data.Store({
    ...
    */
	...
	assertValue: Ext4.emptyFn,
    setValue: function (value, action) {
        var me = this,
        	st = me.store;
        if(Ext4.isString(value)) {value = value.split(me.delimiter)};
        if(me.tempValue) {
    	    var picker = me.getPicker(),
	        	oldPr = picker.preserveScrollOnRefresh;
			value = Ext4.Array.unique(value.concat(me.tempValue));
			...
			me.store.data.addAll(Ext4.Array.filter(me.valueStore.data.items, function(i) {return (Ext4.Array.indexOf(val,i.data[me.valueField]) < 0)}));
    ...
    getRawValue: function () {
        return Ext4.value(this.rawValue, '');
    },
    doRawQuery: function() {
     	var me = this,
			qe;
    ...
    onFocus: function() {
    	...
    	if(view.emptyEl) {
	    	view.emptyEl.setVisibilityMode(Ext4.dom.AbstractElement.DISPLAY);
    		view.emptyEl.hide()
    	}
    	me.view.focus();
    },
    ...
    buildKeyNav: function() {
    	 var me = this,
            ...
            picker = me.getPicker();
		return  new Ext4.view.BoundListKeyNav(picker.el, {
                boundList: picker,
                forceKeyDown: true,
                tab: function(e) {
  	...
  	onExpand: function() {
        ...
        Ext4.defer(keyNav.enable, 3, keyNav); //wait a bit so it doesn't react to the down arrow opening the picker
    ...
    afterComponentLayout : function() {
        ...
        if (!me.view) {
            ...
            del = function(e) {
                ...
                if(selected) {
	                var idx = Ext4.Array.indexOf(me.view,me.view.getNode(selected));
            ...
            me.view = new Ext4.ux.ComboView(Ext4.apply({
                store: me.valueStore,
           ...
           var boxKeyNav=  me.boxKeyNav = new Ext4.view.BoundListKeyNav(me.view.el, {
                boundList: me.view,
                forceKeyDown: true,
                down : function(e) {
                ...
                enter: function(e) {
					...
					if (me.multiSelect && me.createNewOnEnter == true && e.getKey() == e.ENTER  && (rawValue = e.target.value) && (!Ext4.isEmpty(rawValue))) {
            ...
            Ext4.defer(boxKeyNav.enable, 1, boxKeyNav);
        }
    },
    onDestroy: function() {
        var me = this;
        if(me.view) {
            Ext4.destroy(me.view, me.boxKeyNav);
        }
        me.callParent(arguments);
    }
});
