 ...
 *
 */
Ext4.define('Ext4.ux.form.field.BoxSelect203', {
    extend:'Ext.form.field.ComboBox',
    alias: ['widget.comboboxselect203', 'widget.boxselect203'],
    ...
    fieldSubTpl: [
        ...
        '<tpl if="value"> value="{[Ext4.util.Format.htmlEncode(values.value)]}"</tpl>',
     ...
     * @private
     */
    componentLayout: 'boxselectfield203',
    ...
    initComponent: function() {
        ...
        if (typeAhead && !me.editable) {
            Ext4.Error.raise('If typeAhead is enabled the combo must be editable: true -- please change one of those settings.');
        }
        ...
        me.selectionModel = new Ext4.selection.Model({
            store: me.valueStore,
            mode: 'MULTI',
            lastFocused: null,
        ...
        if (!Ext4.isEmpty(me.delimiter) && me.multiSelect) {
    ...
    onBindStore: function(store, initial) {
        ...
        if (store) {
            me.valueStore = new Ext4.data.Store({
                model: store.model,
                proxy: {
                    type: 'memory'
                }
    ...
    onDestroy: function() {
        ...
        Ext4.destroyMembers(me, 'valueStore', 'selectionModel');
    ...
    afterRender: function() {
        ...
        if (Ext4.supports.Placeholder && me.inputEl && me.emptyText) {
        ...
        if (me.grow) {
            if (Ext4.isNumber(me.growMin) && (me.growMin > 0)) {
                me.listWrapper.applyStyles('min-height:'+me.growMin+'px');
            }
            if (Ext4.isNumber(me.growMax) && (me.growMax > 0)) {
    ...
    onLoad: function() {
        ...
        if (valueStore) {
            if (!Ext4.isEmpty(me.value) && (valueStore.getCount() == 0)) {
    ...
    onListSelectionChange: function(list, selectedRecords) {
        ...
        if ((me.ignoreSelection <= 0) && me.isExpanded) {
            ...
            valueStore.each(function(rec) {
                if (Ext4.Array.contains(selectedRecords, rec) || me.isFilteredRecord(rec)) {
                    mergedRecords.push(rec);
                }
            });
            mergedRecords = Ext4.Array.merge(mergedRecords, selectedRecords);

            i = Ext4.Array.intersect(mergedRecords, valueStore.getRange()).length;
    ...
    onKeyUp: function(e, t) {
        ...
        if (me.multiSelect && (me.delimiterRegexp && me.delimiterRegexp.test(rawValue)) ||
            ...
            rawValue = Ext4.Array.clean(rawValue.split(me.delimiterRegexp));
            me.inputEl.dom.value = '';
    ...
    onTypeAhead: function() {
        ...
        if (me.filterPickList) {
            var fn = this.createFilterFn(displayField, inputElDom.value, true, false, false);
            ...
            record = me.store.findRecord(displayField, inputElDom.value, 0, true, false, false);
        }
    ...
    getMultiSelectItemMarkup: function() {
        ...
        if (!me.multiSelectItemTpl) {
            if (!me.labelTpl) {
                me.labelTpl = Ext4.create('Ext.XTemplate',
                    '{[values.' + me.displayField + ']}'
                );
            } else if (Ext4.isString(me.labelTpl) || Ext4.isArray(me.labelTpl)) {
                me.labelTpl = Ext4.create('Ext.XTemplate', me.labelTpl);
            }
        ...
        return this.getTpl('multiSelectItemTpl').apply(Ext4.Array.pluck(this.valueStore.getRange(), 'data'));
    },
    ...
    addValue: function(value) {
        var me = this;
        if (value) {
            me.setValue(Ext4.Array.merge(me.value, Ext4.Array.from(value)));
        }
    },
    ...
    removeValue: function(value) {
        ...
        if (value) {
            me.setValue(Ext4.Array.difference(me.value, Ext4.Array.from(value)));
        }
    },
    ...
    setValue: function(value, doSelect, skipLoad) {
        ...
        if (Ext4.isString(value) && me.multiSelect) {
            value = value.split(me.delimiter);
        }
        value = Ext4.Array.from(value, true);
    ...
    getSubmitData: function() {
        ...
        if (me.multiSelect && me.encodeSubmitValue && val && val[me.name]) {
            val[me.name] = Ext4.encode(val[me.name]);
        }
    ...
    assertValue: function() {
        var me = this,
        rawValue = me.inputEl.dom.value,
        rec = !Ext4.isEmpty(rawValue) ? me.findRecordByDisplay(rawValue) : false,
        value = false;

        if (!rec && !me.forceSelection && me.createNewOnBlur && !Ext4.isEmpty(rawValue)) {
            value = rawValue;
        } else if (rec) {
            value = rec;
        }
    ...
    checkChange: function() {
        if (!this.suspendCheckChange && !this.isDestroyed) {
            ...
            valueField = me.valueField,
            newValue = Ext4.Array.map(Ext4.Array.from(me.value), function(val) {
                if (val.isModel) {
            ...
            if (!isEqual || ((newValue.length > 0 && valueStore.getCount() < newValue.length))) {
                ...
                if (Ext4.isArray(me.valueModels)) {
    ...
    applyEmptyText : function() {
        ...
        if (me.rendered && emptyText) {
            isEmpty = Ext4.isEmpty(me.value) && !me.hasFocus;
            inputEl = me.inputEl;
            if (isEmpty) {
                inputEl.dom.value = '';
 ...
 */
Ext4.define('Ext4.ux.layout.component.field.BoxSelectField203', {
    /* Begin Definitions */
    alias: ['layout.boxselectfield203'],
    extend: 'Ext.layout.component.field.Trigger',

    /* End Definitions */

    type: 'boxselectfield203',
