/*
 * From:
 *   http://dnorman.github.com/ExtJS-Patches/example/ColumnAutoWidthPlugin.html
 */
(function(){
    
var squishCls = Ext4.id(null,'colAutoWidth');
    
Ext4.util.CSS.createStyleSheet([
...
Ext4.define('Ext.ux.ColumnAutoWidthPlugin', {
    ...
    constructor: function(config) {
        ...
        me.viewChangeDT = new Ext4.util.DelayedTask(function(){ me.refresh() });
    ...
    getTableResizers: function() {
        var els = this.grid.getView().getEl().query( '.' + Ext4.baseCSSPrefix + 'grid-table-resizer');
    
        // Grouping feature - first table wraps everything and can be ignored
        if (els.length > 1 && Ext4.fly(els[0]).contains(els[1])) {
            els.shift();
        }
    ...
    getColumnResizers: function(column, config) {
        ...
        var els = this.grid.getEl().query( '.' + Ext4.baseCSSPrefix + 'grid-col-resizer-' + column.id);
 
        // Grouping feature - first table wraps everything and needs to be ignored
        if (els.length > 1 && Ext4.fly(els[0]).parent('table').contains(els[1])) {
            els.shift();
        }
    ...
    getHeaderWidth: function(column){
        //var el = this.grid.el.down( '#' + column.id + ' .' + Ext4.baseCSSPrefix + 'column-header-inner' );
        if(column.el){
    ...
    doAutoSize: function( resizeCols ){
        ...
        Ext4.batchLayouts(function(){
            ...
            Ext4.each(resizeCols, function(col){
                var els = me.getColumnResizers(col), newWidth = 0;
                
                if( col.el ) newWidth = Ext4.num(col.el.dom.scrollWidth,0);
                
                Ext4.each(els, function(el) {
            ...
            Ext4.each( tableResizers , function(el) {
                el = Ext4.fly(el).removeCls( squishCls );
            });
